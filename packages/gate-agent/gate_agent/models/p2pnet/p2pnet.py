...
# util.misc is only needed by the training-time criterion. For inference we
# only call model.forward() with a plain torch.Tensor, so we stub these out.
NestedTensor = torch.Tensor  # type: ignore[assignment]
def is_dist_avail_and_initialized():
    return False
def get_world_size():
    return 1
def accuracy(*a, **k):  # noqa
    return None
def nested_tensor_from_tensor_list(t):  # noqa
    return t
def interpolate(*a, **k):  # noqa
