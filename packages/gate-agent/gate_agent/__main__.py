...
def _build_source(cfg: GateConfig) -> EventSource:
    if cfg.mode == "manual":
    ...
    if cfg.mode == "video-file":
        ...
        return VideoFileSource(path=cfg.video_path, loop=cfg.video_loop, tracking=cfg.tracking)
    if cfg.mode == "webcam":
        if not cfg.tracking:
            raise SystemExit("webcam mode requires `tracking` in config")
        from .sources.webcam import WebcamSource

        return WebcamSource(index=cfg.webcam_index, tracking=cfg.tracking)
